...
class Chaturbate:
    def __init__(self, user_slug):
        self.user_slug = user_slug
        self.user_stream = ''
        self._get_streams()
        self.save_path = f'recordings/{self.user_slug}/{datetime.datetime.now().strftime("%B")}'

    def online_status(self):
        return self.user_stream

    def _get_streams(self):
        stream_source = requests.get(f'https://chaturbate.com/api/chatvideocontext/{self.user_slug}/').json()[
            'hls_source']
        if stream_source:
